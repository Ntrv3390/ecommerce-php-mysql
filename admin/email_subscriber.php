...
if(isset($_GET['type']) && $_GET['type']!=''){
	...
	if($type=='status'){
		...
		$update_status_sql="update `email_subscribe` set `status`='$status' where `id`='$id'";
...
if(isset($_GET['type']) && $_GET['type']!=''){
	...
	if($type=='delete'){
		$id=get_safe_value($con,$_GET['id']);
		$delete_sql="delete from email_subscribe where id='$id'";
...
$sql1="select * from email_subscribe";
if(isset($_GET['search_submit']))
{
    $search = get_safe_value($con,$_GET['search']);
    if($search!=''){
        $sql1.=" where email_subscribe.id like '%$search%' or email_subscribe.email like '%$search%' order by id desc";
    }
}else{
	...
	<div class="orders">
	   <div class="row">
		  <div class="col-xl-12">
			 <div class="card">
				<div class="card-body">
				   <h4 class="box-title">Newsletter Subscribers </h4>
				   <form method="get">
							<div class="card-body card-block">
                                <div class="form-group" style="margin-top: -50px; margin-left:400px;">
									<input  type="text" style="width:70%;margin-bottom:-5px;height:45px;margin-left:58px;" name="search" placeholder="Search in subscribers - id, email..." class="form-control">
