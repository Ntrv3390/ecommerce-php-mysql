...
$promo_code=get_safe_value($con,$_POST['promo_code']);
$res=mysqli_query($con,"select * from coupon_master where coupon_code='$promo_code' and status='1'");
...
if($count>0){
	...
	if($cart_min_value>$cart_total){
		$jsonArr=array('is_error'=>'yes','result'=>$cart_total,'dd'=>'Your total must be more than '.$cart_min_value.' to apply this code.');
	}else{
